...
{foreach from=$journals item=journal}
	...
	<a class="searchCatTitle" href="{url journal=$journal->getPath()}" class="action">{$journal->getLocalizedTitle()|escape}</a>
	{if $journal->getLocalizedDescription()}
		<p>{$journal->getLocalizedDescription()|nl2br}</p>
	{/if}

	<p><!-- <a class="searchCatLink" href="{url journal=$journal->getPath()}" class="action">{translate key="site.journalView"}</a>--><a class="searchCatLink" href="{url journal=$journal->getPath() page="issue" op="current"}" class="action">{translate key="site.journalCurrent"}</a><a class="searchCatLink" href="{url journal=$journal->getPath() page="user" op="register"}" class="action">{translate key="site.journalRegister"}</a></p>
	</div>
{/foreach}
