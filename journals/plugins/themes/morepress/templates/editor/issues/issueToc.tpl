...
<table width="100%" class="listing" id="issueToc-{$sectionKey|escape}">
	...
	<tr id="article-{$article->getPublishedArticleId()|escape}" class="data">
		...
		<td class="drag shareparent">{if !$isLayoutEditor}<a href="{url op="submission" path=$articleId}" class="action" style="padding:0;display:inline;">{/if}{$article->getLocalizedTitle()|strip_tags|truncate:60:"..."}{if !$isLayoutEditor}</a> <a href="#!" class="sharelink larger toc" data-clipboard-text="{$article->getLocalizedTitle()|strip_tags}"><i class="fa fa-clipboard" aria-hidden="true"></i><span class="shareoriginal toc"></span><span class="sharesuccess toc"></span></a>{/if}</td>
    ...
    <td class="shareparent">10.15291/{$currentJournal->getInitials($currentLocale)|lower}.{$article->getId()}</td>
	</tr>
	{/foreach}
</table>
{foreachelse}
